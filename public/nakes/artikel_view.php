...
/**
 * Asumsi:
 *  - Semua cover disimpan di folder: public/uploads/artikels/
 *  - Di DB, kolom cover_image bisa berisi:
 *       - "uploads/artikels/cover_1_xxx.jpg"
 *       - atau path lain; yang penting ada nama filenya.
 *  - Di web, gambar diakses via:
 *       BASE_URL . "/uploads/artikels/{nama_file}"
 */
$coverUrl = null;
if ($artikel && !empty($artikel['cover_image'])) {
    $fileName = basename(trim($artikel['cover_image'])); // ambil nama file saja

    if ($fileName !== '') {
        $coverUrl = rtrim(BASE_URL, '/') . '/uploads/artikels/' . $fileName;
    }
}
?>
<!DOCTYPE html>
